...
public class WxCpJedisConfigStorage implements WxCpConfigStorage {
	...
	private volatile String jedisSuffix; // AcckenToken给每个应用添加一个后缀
	...
	public String getAccessToken() {
		try (Jedis jedis = this.jedisPool.getResource()) {
			return jedis.get(ACCESS_TOKEN_KEY + "_" + this.jedisSuffix);
		}
	}
	...
	public boolean isAccessTokenExpired() {
		try (Jedis jedis = this.jedisPool.getResource()) {
			String expiresTimeStr = jedis.get(ACCESS_TOKEN_EXPIRES_TIME_KEY + "_" + this.jedisSuffix);
	...
	public void expireAccessToken() {
		try (Jedis jedis = this.jedisPool.getResource()) {
			jedis.set(ACCESS_TOKEN_EXPIRES_TIME_KEY + "_" + this.jedisSuffix, "0");
		}
	}
	...
	public synchronized void updateAccessToken(String accessToken, int expiresInSeconds) {
		try (Jedis jedis = this.jedisPool.getResource()) {
			jedis.set(ACCESS_TOKEN_KEY + "_" + this.jedisSuffix, accessToken);

			jedis.set(ACCESS_TOKEN_EXPIRES_TIME_KEY + "_" + this.jedisSuffix, (System.currentTimeMillis() + (expiresInSeconds - 200) * 1000L) + "");
		}
	}
	...
	public String getJsapiTicket() {
		try (Jedis jedis = this.jedisPool.getResource()) {
			return jedis.get(JS_API_TICKET_KEY + "_" + this.jedisSuffix);
		}
	}
	...
	public boolean isJsapiTicketExpired() {
		...
		try (Jedis jedis = this.jedisPool.getResource()) {
			String expiresTimeStr = jedis.get(JS_API_TICKET_EXPIRES_TIME_KEY + "_" + this.jedisSuffix);
	...
	public void expireJsapiTicket() {
		try (Jedis jedis = this.jedisPool.getResource()) {
			jedis.set(JS_API_TICKET_EXPIRES_TIME_KEY + "_" + this.jedisSuffix, "0");
		}
	}
	...
	public synchronized void updateJsapiTicket(String jsapiTicket, int expiresInSeconds) {
		...
		try (Jedis jedis = this.jedisPool.getResource()) {
			jedis.set(JS_API_TICKET_KEY + "_" + this.jedisSuffix, jsapiTicket);

			jedis.set(JS_API_TICKET_EXPIRES_TIME_KEY + "_" + this.jedisSuffix, (System.currentTimeMillis() + (expiresInSeconds - 200) * 1000L + ""));
		}
	...
	public String getJedisSuffix() {
		return jedisSuffix;
	}

	public void setJedisSuffix(String jedisSuffix) {
		this.jedisSuffix = jedisSuffix;
	}
