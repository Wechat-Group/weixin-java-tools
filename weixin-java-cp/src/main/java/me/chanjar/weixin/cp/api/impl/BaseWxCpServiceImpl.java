...
public abstract class BaseWxCpServiceImpl<H, P> implements WxCpService, RequestHttp<H, P> {
  ...
  public String getAgentJsapiTicket(boolean forceRefresh) throws WxErrorException {
    ...
    if (this.configStorage.isAgentJsapiTicketExpired()) {
      synchronized (this.globalAgentJsapiTicketRefreshLock) {
        if (this.configStorage.isAgentJsapiTicketExpired()) {
          String responseContent = this.get(WxCpConsts.getQyapiUrl(WxCpService.GET_AGENT_CONFIG_TICKET), null);
  ...
  public String getJsapiTicket(boolean forceRefresh) throws WxErrorException {
    ...
    if (this.configStorage.isJsapiTicketExpired()) {
      synchronized (this.globalJsapiTicketRefreshLock) {
        if (this.configStorage.isJsapiTicketExpired()) {
          String responseContent = this.get(WxCpConsts.getQyapiUrl(WxCpService.GET_JSAPI_TICKET), null);
  ...
  public WxCpMessageSendResult messageSend(WxCpMessage message) throws WxErrorException {
    ...
    return WxCpMessageSendResult.fromJson(this.post(WxCpConsts.getQyapiUrl(WxCpService.MESSAGE_SEND), message.toJson()));
  }
  ...
  public WxCpMaJsCode2SessionResult jsCode2Session(String jsCode) throws WxErrorException {
    ...
    String result = this.get(WxCpConsts.getQyapiUrl(JSCODE_TO_SESSION_URL), Joiner.on("&").withKeyValueSeparator("=").join(params));
    return WxCpMaJsCode2SessionResult.fromJson(result);
  }

  @Override
  public String[] getCallbackIp() throws WxErrorException {
    String responseContent = get(WxCpConsts.getQyapiUrl(WxCpService.GET_CALLBACK_IP), null);
  ...
  public String replaceParty(String mediaId) throws WxErrorException {
    ...
    return post(WxCpConsts.getQyapiUrl(WxCpService.BATCH_REPLACE_PARTY), jsonObject.toString());
  }
  ...
  public String replaceUser(String mediaId) throws WxErrorException {
    JsonObject jsonObject = new JsonObject();
    jsonObject.addProperty("media_id", mediaId);
    return post(WxCpConsts.getQyapiUrl(WxCpService.BATCH_REPLACE_USER), jsonObject.toString());
  }

  @Override
  public String getTaskResult(String joinId) throws WxErrorException {
    String url = WxCpConsts.getQyapiUrl(WxCpService.BATCH_GET_RESULT + joinId);
    return get(url, null);
  }
